...
export default function SelectedTimes({
  ...
  return (
            ...
            </button>
      {
        timesArray.map((time, i)=>{
          return (
            ...
            onClick={()=> 
            handleAddTime(time)} 
            className="flex items-center justify-center py-2 px-3 border border-blue-100 dark:border-gray-600 rounded-md text-sm gap-2">
              <span>{time}</span>
              <Plus className='w-3 h-3 ml-2'/>
            </button>
    ...
    <div className="p-4">
    <h2 className='font-semibold'>Here is Availability for <span className="text-sm uppercase">{day}</span></h2>
    <div className="py-6 grid grid-cols-3 gap-3">
      {
        selectedTimes.map((time, i)=>{
          return (
            ...
            key={i} 
            className="flex items-center py-2 px-3 border bg-blue-50 dark:text-slate-800 border-red-500 rounded-md text-sm gap-2 justify-center">
              <span>{time}</span>
